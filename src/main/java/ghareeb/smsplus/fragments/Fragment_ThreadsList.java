...
public class Fragment_ThreadsList extends Fragment_LoaderListFromDatabase<ThreadEntity>
{
	class ThreadsAdapter extends ArrayAdapter<ThreadEntity>
	{
		...
		public ThreadsAdapter(Context context, int layoutResourceId, ArrayList<ThreadEntity> data)
		{
		...
		public View getView(int position, View convertView, ViewGroup parent)
		{
			if (data != null)
			{
				...
				ContactViewsHolder holder = null;
				...
				if (row == null)
				{
					...
					holder.quickContactBadge = (QuickContactBadge) row.findViewById(R.id.quickcontact);
					holder.nameNumber = (TextView) row.findViewById(R.id.nameNumberTV);
					holder.dateStatus = (TextView) row.findViewById(R.id.dateStatusTV);
					holder.lastMessage = (TextView) row.findViewById(R.id.lastMessageTV);
					holder.unseenTicker = (TextView) row.findViewById(R.id.unseenMessagesTickerTV);
				...
				else
				{
					...
					if (recent != null)
					{
						...
						if (recent instanceof ReceivedMessage)
							...
							holder.dateStatus.setText(getDateString(((SentMessage) recent).getSendDateTime()));
					}
				}
	...
	public boolean onContextItemSelected(MenuItem item)
	{
		...
		switch (item.getItemId())
		{
			case R.id.delete:
				...
				return true;
			case R.id.call:
				ThreadEntity th = items.get(info.position);
	...
	protected CharSequence getEmptyText()
	{
		...
		CharSequence spannable = s.replacePatternsWithImage(resource, -1.0f, getActivity());
